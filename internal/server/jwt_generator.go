...
func GenerateNewTokens(
	id string,
	//TODO: Implement user role credentials "credentials []string" (admin, moderator, user)
) (*Tokens, error) {
...
func generateNewAccessToken(
	...
	// Set expiration time.
	expiresTime := time.Now().Add(time.Minute * time.Duration(minutesCount)).Unix()
